...
Controller::Controller(ros::NodeHandle &n) {
  ...
  visual_tools_.reset(new rviz_visual_tools::RvizVisualTools(
			        "/zed_left_camera_frame", "/centerline_follower_visual"));
...
void Controller::mapCallback(
	...
	// Visualize path
	visualizePath(path, rviz_visual_tools::BLUE);

	// Visualize radius
	visualizeRadius(radius, rviz_visual_tools::RED);

	// Publish control message
	pub_.publish(control);
}

void Controller::visualizePath(std::vector<Vec2<>> &path, rviz_visual_tools::colors color) {
...
void Controller::visualizeRadius(int radius, rviz_visual_tools::colors color) {
	...
	for (int i = 0; i < 10; i++) {
		...
		pose.translation().y() += 1 / tan(radius / (float)i);
