...
ColorThresholder::ColorThresholder(ros::NodeHandle &n) {
  ...
  sub_ = it.subscribe(image_topic_, 1, &ColorThresholder::imageCallback, this);

  pub_ =
      n.advertise<opencv_msgs::ConeImageMap>("color_thresh/cone_image_2d", 1);
}

void ColorThresholder::imageCallback(const sensor_msgs::Image::ConstPtr &msg) {
...
std::vector<cv::Rect> ColorThresholder::Threshold(cv_bridge::CvImagePtr &cv_ptr,
                                                  cv::Scalar lower_hsv,
                                                  cv::Scalar upper_hsv) {
...
ColorThresholder::CreateConeMsgArray(std::vector<cv::Rect> &rects,
                                     uint8_t color) {
...
cv::Scalar ColorThresholder::ScalarHSV2BGR(cv::Scalar hsv_scalar) {
  cv::Mat rgb;
